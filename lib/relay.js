...
/** Advertise only addresses served by this listener; wildcard binds list candidates. */
function accessUrls(bound, key, interfaces = networkInterfaces()) {
  let addresses = [bound.address]
  if (bound.address === '0.0.0.0' || bound.address === '::') {
    addresses = Object.values(interfaces).flatMap((list) => list ?? [])
      .filter((iface) => !iface.internal && (
        iface.family === 'IPv4' ||
        (bound.address === '::' && iface.family === 'IPv6' && !iface.scopeid)
      ))
      .map((iface) => iface.address)
    if (!addresses.length) addresses = [bound.address === '::' ? '::1' : '127.0.0.1']
  }
  return [...new Set(addresses)].map((address) => {
    const authority = net.isIP(address) === 6 ? `[${address}]` : address
    return `http://${authority}:${bound.port}/?k=${encodeURIComponent(key)}`
  })
}
 ...
 * @returns {Promise<{ port: number, url: string, urls: string[], accessKey: string, close: () => Promise<void> }>}
 */
export function startRelay(options) {
  ...
  const proxy = (request, response) => {
    ...
    const fail = () => {
      ...
      if (response.headersSent) {
        // A partial body cannot be replaced by a successful-looking error body.
        response.destroy()
      } else {
    ...
    const upstream = http.request(
      ...
      (incoming) => {
        ...
        response.writeHead(incoming.statusCode ?? 502, downstreamHeaders(incoming.headers))
  ...
  const handleUpgrade = (request, socket, head) => {
    ...
    upstream.once('close', () => {
      // On a normal EOF, pipe() ends the client after flushing buffered bytes.
  ...
  const close = () => {
    if (!closePromise) {
      ...
      const socketsClosed = [...sockets].map((socket) => new Promise((done) => {
        socket.once('close', done)
        socket.destroy()
      }))
      closePromise = Promise.all([serverClosed, ...socketsClosed]).then(() => {})
    }
    return closePromise
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      ...
      const urls = accessUrls(bound, accessKey)
      const url = urls[0]
      const loopback = bound.address === '::1' || bound.address.startsWith('127.')
      for (const entry of urls) log(`${loopback ? '本机' : '手机'}访问入口: ${entry}`)
      if (urls.length > 1) log('检测到多个网络地址，请选择与手机所在网络对应的入口；虚拟网卡地址可能无法从手机访问')
...
export { PAIR_PATH, PRIVILEGED_METHODS, accessUrls }
