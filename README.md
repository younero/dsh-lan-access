...
全网卡监听时，如果电脑有 Wi-Fi、VPN、Docker 等多个网络地址，终端会列出候选入口，请选择与手机所在网络对应的地址。指定 `host` 时只打印实际绑定的地址；`host: 127.0.0.1` 的入口仅供本机访问。

关闭或重载插件时会断开现有 HTTP 流和 WebSocket 连接。客户端断开后会释放对应的上游连接；上游响应中途断开时，客户端会收到连接中断。
